...
public class AdoptionApplicationController {
    ...
    public AdoptionApplicationController(AdoptionApplicationRepository applications, AnimalRepository animals, FollowUpRepository followUps, ShelterRepository shelters) {
    ...
    public ResponseEntity<?> apply(@RequestBody AdoptionApplication input) {
        ...
        if (applications.existsByAnimalIdAndAdopterEmailIgnoreCase(input.getAnimalId(), email)) {
    ...
    public ResponseEntity<?> updateStatus(@PathVariable Long id, @RequestParam AdoptionApplication.Status status) {
        ...
        if (status == AdoptionApplication.Status.APPROVED) {
            if (app.getStatus() != AdoptionApplication.Status.PENDING) return ResponseEntity.badRequest().body(Map.of("message", "Only pending applications can be approved."));
            if (animal.getStatus() != Animal.AnimalStatus.ADOPTION_READY) return ResponseEntity.badRequest().body(Map.of("message", "Animal is no longer available for adoption."));
        }
        if (status == AdoptionApplication.Status.REJECTED && app.getStatus() != AdoptionApplication.Status.PENDING) {
        ...
        if (status == AdoptionApplication.Status.COMPLETED) {
            if (app.getStatus() != AdoptionApplication.Status.APPROVED) return ResponseEntity.badRequest().body(Map.of("message", "Only an approved application can be completed."));
            Long shelterId = animal.getShelterId();
            if (shelterId != null) shelters.findById(shelterId).ifPresent(s -> { s.setCurrentOccupancy(Math.max(0, s.getCurrentOccupancy() - 1)); shelters.save(s); });
            animal.setShelterId(null);
            animal.setStatus(Animal.AnimalStatus.ADOPTED);
            animals.save(animal);
            if (followUps.findByAdopterEmailIgnoreCaseOrderByScheduledDateAsc(app.getAdopterEmail()).stream().noneMatch(f -> f.getAdoptionApplicationId().equals(app.getId()))) {
                FollowUp f = new FollowUp();
                f.setAdoptionApplicationId(app.getId());
                f.setAnimalId(animal.getId());
                f.setAdopterEmail(app.getAdopterEmail());
                f.setScheduledDate(LocalDate.now().plusDays(30));
                f.setStatus(FollowUp.Status.PENDING);
                followUps.save(f);
            }
            for (AdoptionApplication other : applications.findAllByOrderByAppliedAtDesc()) {
                if (other.getAnimalId().equals(animal.getId()) && !other.getId().equals(app.getId()) && other.getStatus() == AdoptionApplication.Status.PENDING) {
